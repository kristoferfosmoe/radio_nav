...
class BeaconEstimator(Node):
    ...
    def __init__(self):
        super().__init__('beacon_estimator')
    ...
    def gps_callback(self, msg):
        ...
        self.get_logger().debug(f'Received GPS: {msg.latitude}, {msg.longitude}')
        ...
        if self.current_distance is not None:  # Only store positions when we have a corresponding range
            ...
            # self.get_logger().info(f'Added GPS position: {msg.latitude}, {msg.longitude}')
    ...
    def uwb_callback(self, msg):
        ...
        if self.current_gps is not None:  # Only store ranges when we have a corresponding position
            ...
            # self.get_logger().info(f'Added UWB range: {msg.data}')
    ...
    def _multilaterate(self, positions: List[Tuple[float, float]], ranges: List[float]) -> Tuple[float, float]:
        ...
        # Initial guess is our current location - THIS IS IMPORTANT!
    ...
    def estimate_beacon_location(self):
        if self.current_gps is None or self.current_distance is None:
            ...
            # self.get_logger().info(f'UWB distance: {distance} meters = {distance_in_degrees:.8f} decimal degrees')
